...
from zomato.models import User
...
class AuthenticationService:
    ...
    def is_authenticated(self):
        if self.session.current_user is None:
            raise zomatoExit("User is not logged in.")

    def signup(self, username: str, password: str):
        try:
            user = User.create(username=username, password=password)
        except pwee.IntegrityError as e:
            raise zomatoExit(f"User '{username}' already exists.") from e
    ...
    def login(self, username: str, password: str):
        try:
            ...
            raise zomatoExit("Check username and password") from e
